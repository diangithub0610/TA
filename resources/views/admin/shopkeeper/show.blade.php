                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-2">
                                    <strong>Alamat:</strong> {{ $transaksi->alamat->full_address ?? 'N/A' }}
                                </div>
                                <div class="mb-2">
                                ...
                                <div class="row align-items-center py-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                                    <div class="col-md-6">
                                        <h6 class="fw-bold mb-1">
                                            {{ $detail->detailBarang->barang->nama_barang ?? 'Produk tidak ditemukan' }}</h6>
                        ...
                        <div class="row">
                            <div class="col-md-6 offset-md-6">
                                <div class="mb-2 d-flex justify-content-between">
                                    <span>Subtotal</span>
                                    <span>Rp{{ number_format($transaksi->detailTransaksi->sum(function ($detail) {
        return $detail->kuantitas * $detail->harga; }), 0, ',', '.') }}</span>
                                </div>
                                <div class="mb-2 d-flex justify-content-between">
                                    <span>Ongkir</span>
                                ...
                                <div class="d-flex justify-content-between fw-bold fs-5">
                                    <span>Total</span>
                                    <span>Rp{{ number_format($transaksi->detailTransaksi->sum(function ($detail) {
        return $detail->kuantitas * $detail->harga; }) + $transaksi->ongkir - ($transaksi->diskon ?? 0), 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <div class="mb-2">
                                    ...
                                    {{ ucfirst($transaksi->metode_pembayaran ?? 'Transfer Bank') }}
                                </div>
        ...
        'dibatalkan' => 'Dibatalkan',
                        ...
                        <div class="d-flex gap-2">
                            @if ($transaksi->status == 'menunggu_konfirmasi')
                                <button type="button" class="btn btn-dark" onclick="terimaTransaksi('{{ $transaksi->kode_transaksi }}')">
                                    Konfirmasi Pesanan
                                </button>
                                <button type="button" class="btn btn-danger" onclick="tolakTransaksi('{{ $transaksi->kode_transaksi }}')">
                                    Tolak Pesanan
                                </button>
                            @elseif($transaksi->status == 'diproses')
                                <button type="button" class="btn btn-dark" onclick="updateStatus('{{ $transaksi->kode_transaksi }}', 'dikirim')">
                                    Kirim Pesanan
                                </button>
                            @elseif($transaksi->status == 'dikirim')
                                <button type="button" class="btn btn-dark" onclick="updateStatus('{{ $transaksi->kode_transaksi }}', 'selesai')">
                                    Selesaikan Pesanan
                                </button>
                            ...
                            <a href="{{ route('admin.pengiriman.show', $transaksi->pengiriman->id_pengiriman)}}" class="btn btn-primary">
                                Detail Pengiriman
                            </a>
